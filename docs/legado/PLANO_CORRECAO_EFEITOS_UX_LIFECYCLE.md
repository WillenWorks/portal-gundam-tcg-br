...
| P4 — ciclo de vida / link | ✅ | Merge de `fix/simulador-reconecta-partida-terminada` (`queueStatusFor`/`deleteMatch`/`resignMatch` sem oponente — link não reconecta em partida terminada). Novo: `AUTO_FORFEIT_MS` (5min) em `onTurnTimeout` — sem ping/ação do assento que precisa decidir, o servidor encerra por abandono em vez de jogar sozinho pra sempre (era o "jogo rodando ininterrupto"). Resign / deck-out / dano sem shield já encerravam via `resignMatch` / motor. Sweep de 10min já removia a partida terminada. +2 testes (37 no matchStore) |
