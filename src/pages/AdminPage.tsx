...
export default function AdminPage() {
  ...
  useEffect(() => {
    setCardForm(selectedCard ? { ...selectedCard } : emptyCardForm);
  }, [selectedCard]);
  ...
  const submitCard = async () => {
    ...
    if (selectedCardId) await api.updateCard(selectedCardId, payload);
  ...
  const submitRule = async () => {
    ...
    const payload = {
      ...
      sourceType:
        ruleForm.source === "Official FAQ"
          ? "OFFICIAL_FAQ"
          : ruleForm.source === "Community Explainer"
            ? "COMMUNITY_EXPLAINER"
            : "OFFICIAL_RULES",
    ...
    if (selectedRuleId) await api.updateRuling(selectedRuleId, payload);
    else await api.createRuling(payload);
    await loadAll();
    setSelectedRuleId("");
  ...
  const submitTournament = async () => {
    ...
    if (selectedTournamentId) await api.updateTournament(selectedTournamentId, payload);
        ...
        <Card className="panel-cut rounded-none border-primary/30 bg-gradient-to-br from-slate-900 to-cyan-950/20 text-white">
          <CardContent className="flex flex-col gap-4 p-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h2 className="mt-2 font-heading text-5xl uppercase leading-none">CRUD remoto com Prisma em runtime</h2>
              <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300">
                O admin agora opera via backend real. Você pode logar com o usuário seed, editar dados e gravar tudo no PostgreSQL.
              </p>
            </div>
