...
type AdminUser = { id: string; displayName: string; email: string; username: string; role: string; isActive: boolean; isHoster?: boolean };
type AdminSet = any;
type AdminCard = any;
...
export default function AdminPage() {
  ...
  const sectionLabel = useMemo(() => ({ dashboard: "Visão geral", users: "Usuários", cards: "Cartas", sets: "Coleções", seasons: "Temporadas", taxonomies: location.includes("/admin/media") ? "Mídias" : "Traits", rules: "Rulings", events: "Eventos" }[adminSection] || "Gestão"), [adminSection, location]);
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const failedResources = results.map((result, index) => result.status === "rejected" ? ["usuários", "coleções", "coleções (gestão)", "rulings", "taxonomias", "taxonomias (gestão)", "eventos", "grupos de banimento", "decks públicos", "temporadas"][index] : null).filter(Boolean);
