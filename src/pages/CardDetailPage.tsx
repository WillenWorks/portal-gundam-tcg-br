...
export default function CardDetailPage() {
  ...
  return <PublicShell breadcrumbs={breadcrumbs}>
    <div className="space-y-6">
      ...
      {card ? <>
        <section className="grid gap-6 xl:grid-cols-[minmax(300px,0.62fr)_minmax(0,1.38fr)]">
          ...
          <Card className="panel-cut rounded-none border-primary/30 hero-surface"><CardContent className="space-y-5 p-6">
            ...
            <div><p className="text-xs uppercase tracking-[0.26em] text-slate-400">{card.set?.code || "Impressão sem coleção"} · {card.code}</p><h1 className="mt-2 font-heading text-5xl uppercase leading-none">{card.namePt || card.nameEn}</h1><p className="mt-3 text-sm text-slate-400">{card.nameEn}{card.namePt ? ` · ${card.namePt}` : ""}</p></div>
