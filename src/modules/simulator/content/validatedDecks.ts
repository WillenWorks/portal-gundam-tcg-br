 ...
 *    (`catalog:*`), fora do alcance deste módulo puro — os fixtures ST01-08 são
 *    os starter decks oficiais, sem carta banida.
 * 2. **Cobertura de efeitos** — toda carta do deck é `implementada`, `vanilla`
 *    ou `implementada*` no dashboard `pnpm catalog:coverage` (docs/48).
 */
...
export const VALIDATED_DECKS: Record<string, ValidatedDeck> = {
  ...
  ST06: { id: "ST06", label: 'ST06 "Red Gundam & GQuuuuuuX"', build: buildSt06DeckList },
  ST07: { id: "ST07", label: 'ST07 "Celestial Being"', build: buildSt07DeckList },
  ST08: { id: "ST08", label: 'ST08 "Hathaway\'s Flash"', build: buildSt08DeckList },
};
