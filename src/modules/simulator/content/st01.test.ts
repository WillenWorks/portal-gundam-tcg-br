...
describe("EffectSpecs reais do ST01 (docs/18 passo 3)", () => {
  ...
  describe("ST01-002 Gundam (MA Form) — When Paired (White Base Team Pilot): Draw 1", () => {
    it("compra 1 carta quando pareado com Piloto (White Base Team)", () => {
      ...
      const pilotId = placeCard(state, "A", ST01_CARD_DEFS.AMURO_RAY, "battleArea");
      const unitId = placeCard(state, "A", ST01_CARD_DEFS.GUNDAM_MA_FORM, "battleArea", { pairedPilotId: pilotId });
    ...
    it("não compra nada se o Piloto pareado não tem o trait (White Base Team)", () => {
      const state = freshGame();
      const pilotId = placeCard(state, "A", ST01_CARD_DEFS.SULETTA_MERCURY, "battleArea"); // trait (Academy), não (White Base Team)
      const unitId = placeCard(state, "A", ST01_CARD_DEFS.GUNDAM_MA_FORM, "battleArea", { pairedPilotId: pilotId });
      const ctx: EffectContext = { state, controller: "A", sourceInstanceId: unitId, turnNumber: state.turnNumber, targets: {} };
  ...
  it("ST01-004 Guntank — Deploy: resta o alvo escolhido", () => {
    const state = freshGame();
    const guntankId = placeCard(state, "A", ST01_CARD_DEFS.GUNTANK, "battleArea");
    const targetId = placeCard(state, "B", ST01_CARD_DEFS.GM, "battleArea");
  ...
  it("ST01-006 Gundam Aerial (Score Six) — When Paired: alvo recebe AP-3 nesse turno", () => {
    const state = freshGame();
    const sourceId = placeCard(state, "A", ST01_CARD_DEFS.AERIAL_SCORE_SIX, "battleArea");
    const targetId = placeCard(state, "B", ST01_CARD_DEFS.GUNCANNON, "battleArea");
  ...
  describe("ST01-010 Amuro Ray", () => {
    it("Burst: a própria carta (revelada como shield) vai pra mão", () => {
      const state = freshGame();
      const amuroId = placeCard(state, "A", ST01_CARD_DEFS.AMURO_RAY, "shields");
    ...
    it("When Paired: resta o alvo escolhido", () => {
      const state = freshGame();
      const amuroId = placeCard(state, "A", ST01_CARD_DEFS.AMURO_RAY, "battleArea");
      const targetId = placeCard(state, "B", ST01_CARD_DEFS.GM, "battleArea");
  ...
  describe("ST01-011 Suletta Mercury", () => {
    it("Burst: a própria carta (revelada como shield) vai pra mão", () => {
      const state = freshGame();
      const sulettaId = placeCard(state, "A", ST01_CARD_DEFS.SULETTA_MERCURY, "shields");
    ...
    it("Attack: ativa (seta active) o Resource escolhido", () => {
      const state = freshGame();
      const sulettaId = placeCard(state, "A", ST01_CARD_DEFS.SULETTA_MERCURY, "battleArea");
      const resourceId = placeCard(state, "A", ST01_CARD_DEFS.RESOURCE, "resourceArea", { rested: true });
  ...
  describe("ST01-012 Thoroughly Damaged — Main: 1 dano no alvo rested", () => {
    it("aplica 1 dano sem destruir se o alvo sobrevive", () => {
      const state = freshGame();
      const sourceId = placeCard(state, "A", ST01_CARD_DEFS.GM, "battleArea"); // fonte fictícia, só pro ctx
      const targetId = placeCard(state, "B", ST01_CARD_DEFS.GUNCANNON, "battleArea", { rested: true }); // HP4
      const ctx: EffectContext = { state, controller: "A", sourceInstanceId: sourceId, turnNumber: state.turnNumber, targets: { target: [targetId] } };
    ...
    it("destrói o alvo se o dano bater o HP efetivo (Comprehensive Rules 5-5-2)", () => {
      const state = freshGame();
      const sourceId = placeCard(state, "A", ST01_CARD_DEFS.GM, "battleArea");
      const targetId = placeCard(state, "B", ST01_CARD_DEFS.DEMI_TRAINER, "battleArea", { rested: true, damage: 0 }); // HP1
      const ctx: EffectContext = { state, controller: "A", sourceInstanceId: sourceId, turnNumber: state.turnNumber, targets: { target: [targetId] } };
  ...
  it("ST01-013 Kai's Resolve — Main: alvo recupera 3 HP", () => {
    const state = freshGame();
    const sourceId = placeCard(state, "A", ST01_CARD_DEFS.GM, "battleArea");
    const targetId = placeCard(state, "A", ST01_CARD_DEFS.GUNCANNON, "battleArea", { damage: 4 }); // HP4, quase destruída
    const ctx: EffectContext = { state, controller: "A", sourceInstanceId: sourceId, turnNumber: state.turnNumber, targets: { target: [targetId] } };
  ...
  describe("ST01-015 White Base / ST01-016 Asticassia — Burst + Deploy", () => {
    it("Burst: a própria carta (revelada como shield) se deploya na Base Section", () => {
      const state = freshGame();
      const baseId = placeCard(state, "A", ST01_CARD_DEFS.WHITE_BASE, "shields");
    ...
    it("Deploy: 1 Shield escolhido vai pra mão", () => {
      const state = freshGame();
      const baseId = placeCard(state, "A", ST01_CARD_DEFS.WHITE_BASE, "baseSection");
    ...
    it("Asticassia segue o mesmo padrão de Burst (self -> baseSection)", () => {
      const state = freshGame();
      const baseId = placeCard(state, "A", ST01_CARD_DEFS.ASTICASSIA, "shields");
      const ctx: EffectContext = { state, controller: "A", sourceInstanceId: baseId, turnNumber: state.turnNumber, targets: {} };
    ...
    it("Asticassia segue o mesmo padrão de Deploy (1 Shield -> mão)", () => {
      const state = freshGame();
      const baseId = placeCard(state, "A", ST01_CARD_DEFS.ASTICASSIA, "baseSection");
      const shieldId = state.players.A.shields[0].instanceId;
      const ctx: EffectContext = { state, controller: "A", sourceInstanceId: baseId, turnNumber: state.turnNumber, targets: { shield: [shieldId] } };
  ...
  describe("ST01-015 White Base — Activate·Main ②: deploy de token condicional por Units em campo (docs/18 lacunas #3/#4)", () => {
    it("custo ④②: resta 2 Recursos active e, com 0 Units em campo, deploya 1 token [Gundam] (AP3/HP3)", () => {
      const state = freshGame();
      const baseId = placeCard(state, "A", ST01_CARD_DEFS.WHITE_BASE, "baseSection");
      const r1 = placeCard(state, "A", ST01_CARD_DEFS.RESOURCE, "resourceArea");
      const r2 = placeCard(state, "A", ST01_CARD_DEFS.RESOURCE, "resourceArea");
      const ctx: EffectContext = { state, controller: "A", sourceInstanceId: baseId, turnNumber: state.turnNumber, targets: {} };
    ...
    it("com 1 Unit em campo deploya [Guncannon]; com 2+ deploya [Guntank]", () => {
      const state = freshGame();
      const baseId = placeCard(state, "A", ST01_CARD_DEFS.WHITE_BASE, "baseSection");
      placeCard(state, "A", ST01_CARD_DEFS.RESOURCE, "resourceArea");
      placeCard(state, "A", ST01_CARD_DEFS.RESOURCE, "resourceArea");
      placeCard(state, "A", ST01_CARD_DEFS.RESOURCE, "resourceArea");
      placeCard(state, "A", ST01_CARD_DEFS.RESOURCE, "resourceArea");
      placeCard(state, "A", ST01_CARD_DEFS.GM, "battleArea");
  ...
  it("ST01-016 Asticassia — Activate·Main: resta a Base e dá AP+1 a toda Unit amiga com Link ativo (docs/18 lacuna #5)", () => {
    ...
    const baseId = placeCard(state, "A", ST01_CARD_DEFS.ASTICASSIA, "baseSection");
    const amuroId = placeCard(state, "A", ST01_CARD_DEFS.AMURO_RAY, "battleArea");
    const linkedId = placeCard(state, "A", ST01_CARD_DEFS.GUNDAM, "battleArea", { pairedPilotId: amuroId });
    const unlinkedId = placeCard(state, "A", ST01_CARD_DEFS.GM, "battleArea");
    const ctx: EffectContext = { state, controller: "A", sourceInstanceId: baseId, turnNumber: state.turnNumber, targets: {} };
