...
describe("ST04 — EffectSpecs bespoke", () => {
  it("ST04-001 Aile Strike — 【When Paired･Lv.4+ Pilot】devolve Unit inimiga HP≤4 pra mão", () => {
    ...
    const aileId = placeCard(state, "A", ST04_CARD_DEFS.AILE_STRIKE_GUNDAM, "battleArea");
    const kiraId = placeCard(state, "A", ST04_CARD_DEFS.KIRA_YAMATO, "battleArea"); // Lv.4
    pair(state, aileId, kiraId);
    const enemyId = placeCard(state, "B", ST04_CARD_DEFS.GINN, "battleArea"); // HP2
  ...
  it("ST04-001 Aile Strike — sem Pilot Lv.4+ o efeito não dispara", () => {
    const state = freshGame();
    const aileId = placeCard(state, "A", ST04_CARD_DEFS.AILE_STRIKE_GUNDAM, "battleArea");
    const enemyId = placeCard(state, "B", ST04_CARD_DEFS.GINN, "battleArea");
  ...
  it("ST04-002 Strike Gundam — 【Deploy】compra 1 e descarta 1 escolhida", () => {
    const state = freshGame();
    const strikeId = placeCard(state, "A", ST04_CARD_DEFS.STRIKE_GUNDAM, "battleArea");
  ...
  it("ST04-006 Aegis Gundam — 【Attack】com AP≥5 dá 3 de dano em Unit Lv.5+", () => {
    const state = freshGame();
    const aegisId = placeCard(state, "A", ST04_CARD_DEFS.AEGIS_GUNDAM, "battleArea", {
      statModifiers: [{ stat: "ap", amount: 1, duration: "endOfTurn", appliedOnTurn: state.turnNumber }],
    }); // AP4 + 1 = 5
    const bigEnemyId = placeCard(state, "B", ST04_CARD_DEFS.AILE_STRIKE_GUNDAM, "battleArea"); // Lv.5 HP4
  ...
  it("ST04-009 Miguel's Ginn — 【Destroyed】compra 1 se há outra Link Unit amiga", () => {
    const state = freshGame();
    const ginnId = placeCard(state, "A", ST04_CARD_DEFS.MIGUELS_GINN, "trash");
    // outra Link Unit: Strike Gundam (link [Kira Yamato]) pareada com Kira
    const strikeId = placeCard(state, "A", ST04_CARD_DEFS.STRIKE_GUNDAM, "battleArea");
    const kiraId = placeCard(state, "A", ST04_CARD_DEFS.KIRA_YAMATO, "battleArea");
  ...
  it("ST04-010 Kira Yamato — 【Attack】Unit inimiga fica com AP-2 nesta batalha", () => {
    const state = freshGame();
    const kiraId = placeCard(state, "A", ST04_CARD_DEFS.KIRA_YAMATO, "battleArea");
    const enemyId = placeCard(state, "B", ST04_CARD_DEFS.STRIKE_DAGGER, "battleArea");
  ...
  it("ST04-012 Striker Pack — 【Burst】invoca token Aile Strike se não há token Earth Alliance", () => {
    const state = freshGame();
    const cmdId = placeCard(state, "A", ST04_CARD_DEFS.STRIKER_PACK, "trash");
  ...
  it("ST04-012 Striker Pack — 【Main】sem token Earth Alliance: 'launcher' → T-009, senão (sword / sem escolha) → T-010", () => {
    let state = freshGame();
    let cmdId = placeCard(state, "A", ST04_CARD_DEFS.STRIKER_PACK, "trash");
    ...
    state = freshGame();
    cmdId = placeCard(state, "A", ST04_CARD_DEFS.STRIKER_PACK, "trash");
  ...
  it("ST04-012 Striker Pack — 【Main】NÃO invoca nada se já há um token (Earth Alliance) em campo", () => {
    const state = freshGame();
    const cmdId = placeCard(state, "A", ST04_CARD_DEFS.STRIKER_PACK, "trash");
    placeCard(state, "A", ST04_CARD_DEFS.TOKEN_AILE_STRIKE, "battleArea"); // já tem token EA
    const before = state.players.A.battleArea.length;
  ...
  it("ST04-013 Hawk of Endymion — 【Main】devolve Unit inimiga HP≤3 pra mão", () => {
    const state = freshGame();
    const cmdId = placeCard(state, "A", ST04_CARD_DEFS.HAWK_OF_ENDYMION, "trash");
    const enemyId = placeCard(state, "B", ST04_CARD_DEFS.MOEBIUS_ZERO, "battleArea"); // HP4 -> NÃO passa hp<=3
    const strongEnemy = placeCard(state, "B", ST04_CARD_DEFS.GINN, "battleArea"); // HP2 -> passa
  ...
  it("ST04-014 The Magic Bullet of Dusk — 【Main】Unit amiga Lv≤2 ganha <First Strike>", () => {
    const state = freshGame();
    const cmdId = placeCard(state, "A", ST04_CARD_DEFS.THE_MAGIC_BULLET_OF_DUSK, "trash");
    const allyId = placeCard(state, "A", ST04_CARD_DEFS.GINN, "battleArea"); // Lv.2
  ...
  it("ST04-015 Archangel — 【Activate･Main】paga 2, seta Unit <Blocker> amiga como active e proíbe ataque no turno", () => {
    const state = freshGame();
    const baseId = placeCard(state, "A", ST04_CARD_DEFS.ARCHANGEL, "baseSection");
    [0, 1, 2].forEach(() => placeCard(state, "A", ST04_CARD_DEFS.RESOURCE, "resourceArea"));
    const blockerId = placeCard(state, "A", ST04_CARD_DEFS.MOEBIUS, "battleArea", { rested: true }); // <Blocker>
  ...
  it("ST04-015 Archangel — a Unit setada como active não consegue declarar ataque no mesmo turno", () => {
    ...
    const baseId = placeCard(state, "A", ST04_CARD_DEFS.ARCHANGEL, "baseSection");
    [0, 1, 2].forEach(() => placeCard(state, "A", ST04_CARD_DEFS.RESOURCE, "resourceArea"));
    const blockerId = placeCard(state, "A", ST04_CARD_DEFS.MOEBIUS, "battleArea", { rested: true });
  ...
  it("ST04-011 Athrun Zala — 【When Linked】concede attackTargetRelaxUntilTurn à Unit pareada", () => {
    const state = freshGame();
    const aegisId = placeCard(state, "A", ST04_CARD_DEFS.AEGIS_GUNDAM, "battleArea"); // link [Athrun Zala]
    const athrunId = placeCard(state, "A", ST04_CARD_DEFS.ATHRUN_ZALA, "battleArea", { pairedUnitId: aegisId });
  ...
  it("ST04-011 Athrun Zala — depois da concessão, a Unit pareada mira Unit inimiga ATIVA Lv<=5", () => {
    ...
    const activeEnemyId = placeCard(state, "B", ST04_CARD_DEFS.STRIKE_GUNDAM, "battleArea"); // Lv.4, ativa
  ...
  it("ST04-016 Vesalius — 【Activate･Main】resta a Base e dá AP+1 numa Unit amiga", () => {
    const state = freshGame();
    const baseId = placeCard(state, "A", ST04_CARD_DEFS.VESALIUS, "baseSection");
    const allyId = placeCard(state, "A", ST04_CARD_DEFS.GINN, "battleArea");
